...
function NewUsers() {
    ...
    const fetchNewUsers = async() => {
        await axios.get('http://localhost:4000/api/users/new') 
        .then(response => {
    ...
    const handleApprove = (userId) => {
        ...
        axios.put(`http://localhost:4000/api/user/${userId}/approve`)
            .then(response => {
    ...
    const handleReject = (userId) => {
        ...
        axios.put(`http://localhost:4000/api/user/${userId}/reject`)
        .then(response => {
            // Mettre à jour la liste des utilisateurs après l'approbation
            console.log(response);
            fetchNewUsers();
