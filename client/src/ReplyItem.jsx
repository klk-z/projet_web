  ...
  const fetchNestedReplies = async () => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/reply/${reply._id}/replies`
      );
      ...
      console.error(
        "Erreur lors de la récupération des réponses imbriquées :",
        error
      );
    }
  };
  ...
  const postNestedReply = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:4000/api/reply/reply`,
        nestedReplyData
      );
  ...
  const deleteReply = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:4000/api/reply/${reply._id}`
      );
      if (response.status === 201) {
      ...
      <div>
        ...
        <span>
          {reply.author == user.username ? (
            <button onClick={() => deleteReply()}>Supprimer</button>
          ) : (
            <></>
          )}
        </span>
